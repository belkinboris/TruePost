...
async def consult(channel: "Channel", user_message: str, history: list, rules_text: str = "") -> tuple[str, str | None]:
    """
    ИИ-консультант для настройки стиля.
    Возвращает (ответ, suggested_rule или None).
    """
    system = f"""Ты — консультант по стилю Telegram-канала «{channel.title}».
Твоя задача: понять чего хочет автор и помочь сформулировать конкретные правила для генерации постов.

О канале: {channel.about}
Текущие правила канала: {rules_text or "нет"}

Веди диалог. Задавай уточняющие вопросы. Когда понимаешь конкретное требование — 
в конце ответа добавь строку:
ПРАВИЛО: [конкретное правило для генератора постов]

Примеры правил:
ПРАВИЛО: Не использовать длинное тире (—), только короткое (-)
ПРАВИЛО: Писать только про сделки 2025-2026 года, не упоминать старые
ПРАВИЛО: Начинать каждый пост с конкретной цифры или имени человека

Если ещё не понял достаточно — не добавляй ПРАВИЛО, продолжай диалог."""
